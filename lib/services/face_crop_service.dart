...
originalImage = img.bakeOrientation(originalImage); // <-- add this line

    // Step 1: Align the face using eye landmarks.
    final alignedImage = rotateImage(
      originalImage,
      face,
    );

    // Step 2: Detect the face again after alignment.
    final alignedFace =
        await detectFaceFromDecodedImage(
      alignedImage,
    );

    // Step 3: Build a stable crop around the eyes.
    final cropRect = buildFaceCropRect(
      alignedFace,
      alignedImage,
    );
    ...
    // Step 5: Resize for MobileFaceNet.
    ...
    double left =
        eyeCenter.dx - cropSize / 2;

    double top =
        eyeCenter.dy - cropSize * 0.42;
  ...
  Future<List<File>> generateAugmentedFaces(
    File croppedFace,
  ) async {
